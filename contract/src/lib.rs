...
pub trait EscrowContract {
    ...
    fn create(&self, buyer: ManagedAddress) {
        ...
        require!(payment.deref() > &BigUint::zero(), "Must pay more than 0");
        ...
        self.offer(new_offer_id).set(&offer);
    ...
    #[storage_mapper("reentrancy_guard")]
    fn reentrancy_guard(&self) -> SingleValueMapper<bool>;

    #[endpoint(acceptOffer)]
    fn accept_offer(&self, offer_id: u64) {
        require!(!self.reentrancy_guard().get(), "Reentrant call");
        ...
        self.send().direct_egld(&offer.creator, &offer.amount);

        self.user_offers(&offer.creator).remove(&offer_id);
        self.user_incoming_offers(&caller).remove(&offer_id);
    ...
    fn cancel_offer(&self, offer_id: u64) {
        ...
        self.user_offers(&caller).remove(&offer_id);
        self.user_incoming_offers(&offer.recipient).remove(&offer_id);
